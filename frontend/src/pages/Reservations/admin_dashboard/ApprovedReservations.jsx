...
const ApprovedReservations = () => {
  ...
  const handleExport = async () => {
    ...
    const columnWidths = [40, 45, 45, 30, 130, 30, 50, 29, 34];
      ...
      reader.onloadend = () => {
        ...
        const docDefinition = {
          content: [
            ...
            { text: "Approved Reservations Report", style: "header" },
            {
              table: {
                headerRows: 1,
                widths: columnWidths,
        ...
        pdfDoc.download("approved_reservations.pdf");
      };
    } catch (error) {
  ...
  const handleSearch = () => {
    setLoading(true);
    fetch(`http://localhost:3000/reservation/search-approved?query=${query}`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${user.token}`,
